...
def data_prepare_BERT(file_path, lab2ind, tokenizer, content_col, MAX_LEN):
	...
	# print(labels)
	labels=[]
	for i in range(0,len(df[content_col])):
	  labels.append(0)
	len(labels)
...
config={"data_file": "static/input_data.xlsx", #data file path
             "pretrained_model_path": 'MARBERT_pytorch_verison', #MARBERT checkpoint path
             "model_path": 'finetuned_model', #MARBERT finetuned model path
...
lab2ind = json.load(open('IraqiT123_MARBERT_labels-dict.json'))

def prediction():
	model = BertForSequenceClassification.from_pretrained(model_path, num_labels=5)
	validation_inputs, validation_labels, validation_masks = data_prepare_BERT(data_file, lab2ind, tokenizer, content_col,max_seq_length)
	...
	#result_df.to_excel('emo_rec/result.xlsx')
	tweets=pd.read_excel(data_file)
	tweets['prediction']=result
	to_labels=[]
	for i in range(0,len(tweets)):
		if tweets['prediction'].iloc[i]==0:
			to_labels.append('ANGER')
		elif tweets['prediction'].iloc[i]==1:
			to_labels.append('FAVORITISM')
		elif tweets['prediction'].iloc[i]==2:
			to_labels.append('HAPPINESS')
		elif tweets['prediction'].iloc[i]==3:
			to_labels.append('NONE OF THEM')
		elif tweets['prediction'].iloc[i]==4:
			to_labels.append('SADNESS')
	tweets['class']=to_labels
	#tweets=tweets['tweet']
	#result_df['tweet']=pd.Dataframe(tweets,columns=['tweet'])
	tweets.to_excel('static/result.xlsx')
	return "You can download results from link below:"